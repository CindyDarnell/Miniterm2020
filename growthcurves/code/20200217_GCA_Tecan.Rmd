...
# metafile name
meta <- 'meta_example_Tecan.csv'
# folder with data
folderdata <- '/cloud/project/miniterm/data/'
# datafile name
data <- 'data_example_Tecan.csv'
...
dt <- read.csv(paste(folderdata, data, sep = ''),
               ...
               header = FALSE)
dt0 <- dt
colnames(dt0) <- c('time', mt0[1:96,1])
...
t_mtdt <- c_mtdt[!(c_mtdt$time <= 3 | c_mtdt$time >= 75), ]
...
head(b_mtdt)
## reorganize columns (variable number subtracted to prevent negative values)
b_mtdt$value_mn<- b_mtdt$value_mn - (b_mtdt$b_mn-0.7)
b_mtdt$b_mn<- NULL
## remove blank rows
b_mtdt <- b_mtdt[!b_mtdt$name=='blank',]
## if the value mean is negative, subtract it from the blank mean above
min(b_mtdt$value_mn)
```
